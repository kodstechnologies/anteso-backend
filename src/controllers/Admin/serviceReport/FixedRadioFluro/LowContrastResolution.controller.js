// controllers/LowContrastResolution.js
import mongoose from "mongoose";
import LowContrastResolution from "../../../../models/testTables/FixedRadioFluro/LowContrastResolution.model.js";
...
const create = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        if (existing) {
            await session.abortTransaction();
            return res.status(400).json({ message: "Low Contrast Resolution data already exists for this service" });
        }
        ...
        serviceReport.LowContrastResolutionFixedRadioFlouro = newTest[0]._id;
        await serviceReport.save({ session });
...
const getByServiceId = asyncHandler(async (req, res) => {
    ...
    const testData = await LowContrastResolution.findOne({ serviceId }).lean();

    if (!testData) {
        return res.status(404).json({ message: "No Low Contrast Resolution data found for this service" });
    }
...
const getById = asyncHandler(async (req, res) => {
    ...
    const testData = await LowContrastResolution.findById(testId).lean();

    if (!testData) {
        return res.status(404).json({ message: "Low Contrast Resolution test not found" });
    }
