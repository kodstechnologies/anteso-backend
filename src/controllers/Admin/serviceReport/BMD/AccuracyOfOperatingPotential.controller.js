// controllers/Admin/serviceReport/BMD/AccuracyOfOperatingPotential.controller.js
import mongoose from "mongoose";
import AccuracyOfOperatingPotential from "../../../../models/testTables/BMD/AccuracyOfOperatingPotential.model.js";
...
const create = asyncHandler(async (req, res) => {
  ...
  const { table1, table2, tolerance, toleranceValue, toleranceType, toleranceSign } = req.body;
  
  // Handle both formats: tolerance object or separate fields
  let toleranceObj = tolerance;
  if (!toleranceObj && (toleranceValue !== undefined || toleranceType !== undefined || toleranceSign !== undefined)) {
    toleranceObj = {
  ...
  try {
    ...
    if (testRecord) {
      ...
      if (toleranceObj !== undefined) testRecord.tolerance = toleranceObj;
    } else {
      // Create new
      testRecord = new AccuracyOfOperatingPotential({
        ...
        table1: table1 || [],
        table2: table2 || [],
        tolerance: toleranceObj || { value: "", type: "percent", sign: "both" },
      });
    }
    ...
    serviceReport.accuracyOfOperatingPotentialBMD = testRecord._id;
    await serviceReport.save({ session });
