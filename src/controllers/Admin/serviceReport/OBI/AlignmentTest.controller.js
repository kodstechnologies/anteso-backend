// controllers/Admin/serviceReport/OBI/AlignmentTest.controller.js
import mongoose from "mongoose";
import AlignmentTest from "../../../../models/testTables/OBI/AlignmentTest.model.js";
...
const create = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    let testRecord = await AlignmentTest.findOne({ serviceId }).session(session);

    if (testRecord) {
      // Update existing
      testRecord.testRows = testRows !== undefined ? testRows : testRecord.testRows;
    } else {
      // Create new
      testRecord = new AlignmentTest({
        serviceId,
    ...
    serviceReport.AlignmentTestOBI = testRecord._id;
    await serviceReport.save({ session });
    ...
    console.error("AlignmentTest Create Error:", error);
...
const getById = asyncHandler(async (req, res) => {
  ...
  try {
    const testRecord = await AlignmentTest.findById(testId).lean();
    if (!testRecord) {
...
const update = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const testRecord = await AlignmentTest.findById(testId).session(session);
    if (!testRecord) {
      await session.abortTransaction();
      return res.status(404).json({ success: false, message: "Test record not found" });
    }
    ...
    // Update fields
    if (testRows !== undefined) testRecord.testRows = testRows;
    ...
    if (session) await session.abortTransaction();
    console.error("AlignmentTest Update Error:", error);
    return res.status(500).json({
      success: false,
...
const getByServiceId = asyncHandler(async (req, res) => {
  ...
  try {
    const testRecord = await AlignmentTest.findOne({ serviceId }).lean();
