...
import TimerAccuracy from "../../../../models/testTables/InventionalRadiology/TimerAccuracy.model.js";
...
const create = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    else testRecord = new TimerAccuracy(payload);

    await testRecord.save({ session });

    serviceReport.TimerAccuracyInventionalRadiology = testRecord._id;
    await serviceReport.save({ session });
...
const getById = asyncHandler(async (req, res) => {
  ...
  const rec = await TimerAccuracy.findById(testId).lean().exec();
...
const update = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const rec = await TimerAccuracy.findById(testId).session(session);
    if (!rec) {
      await session.abortTransaction();
    ...
    // Optional tube mismatch protection
    ...
    rec.table1 = table1;
    rec.table2 = table2;
    rec.tolerance = tolerance?.toString().trim() || "";
...
const getByServiceId = asyncHandler(async (req, res) => {
  ...
  const rec = await TimerAccuracy.findOne(query).lean().exec();
