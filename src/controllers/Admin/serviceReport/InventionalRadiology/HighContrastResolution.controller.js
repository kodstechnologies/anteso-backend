...
const create = asyncHandler(async (req, res) => {
    const { measuredLpPerMm, recommendedStandard, tolerance, remark, tubeId } = req.body;
    ...
    try {
        ...
        let testRecord = await HighContrastResolution.findOne({ serviceId, tubeId: tubeIdValue }).session(session);

        const payload = {
            measuredLpPerMm: measuredLpPerMm?.trim() || "",
            recommendedStandard: recommendedStandard?.trim() || "1.50",
        ...
        if (testRecord) {
            ...
            testRecord = new HighContrastResolution(payload);
        }
        await testRecord.save({ session });

        // 4. Link to ServiceReport
        serviceReport.HighContrastResolutionInventionalRadiology = testRecord._id;
        await serviceReport.save({ session });
...
const getById = asyncHandler(async (req, res) => {
    ...
    try {
        const testRecord = await HighContrastResolution.findById(testId)
            .lean()
            .exec();
...
const update = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        const testRecord = await HighContrastResolution.findById(testId).session(session);
        if (!testRecord) {
            await session.abortTransaction();
            return res.status(404).json({ success: false, message: "Test record not found" });
        }
        ...
        // Update fields
        if (measuredLpPerMm !== undefined) testRecord.measuredLpPerMm = measuredLpPerMm?.trim() || "";
        if (recommendedStandard !== undefined) testRecord.recommendedStandard = recommendedStandard?.trim() || "1.50";
...
const getByServiceId = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        const testRecord = await HighContrastResolution.findOne(query).lean().exec();
