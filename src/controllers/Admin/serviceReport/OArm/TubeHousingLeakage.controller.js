// controllers/Admin/serviceReport/OArm/TubeHousingLeakage.controller.js
import mongoose from "mongoose";
import TubeHousingLeakage from "../../../../models/testTables/OArm/TubeHousingLeakage.model.js";
...
const create = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (testRecord) {
      // Update existing
      if (fcd !== undefined) testRecord.fcd = fcd?.toString().trim() || "";
      if (kv !== undefined) testRecord.kv = kv?.toString().trim() || "";
      if (ma !== undefined) testRecord.ma = ma?.toString().trim() || "";
      if (time !== undefined) testRecord.time = time?.toString().trim() || "";
      if (workload !== undefined) testRecord.workload = workload?.toString().trim() || "";
      if (leakageMeasurements !== undefined) testRecord.leakageMeasurements = leakageMeasurements;
      if (toleranceValue !== undefined) testRecord.toleranceValue = toleranceValue?.toString().trim() || "";
      if (toleranceOperator !== undefined) testRecord.toleranceOperator = toleranceOperator?.toString().trim() || "";
      if (toleranceTime !== undefined) testRecord.toleranceTime = toleranceTime?.toString().trim() || "";
      if (remark !== undefined) testRecord.remark = remark?.toString().trim() || "";
    } else {
      // Create new
      testRecord = new TubeHousingLeakage({
        serviceId,
        reportId: serviceReport._id,
        fcd: fcd?.toString().trim() || "",
        kv: kv?.toString().trim() || "",
        ma: ma?.toString().trim() || "",
        time: time?.toString().trim() || "",
        workload: workload?.toString().trim() || "",
        leakageMeasurements: leakageMeasurements || [],
        toleranceValue: toleranceValue?.toString().trim() || "",
        toleranceOperator: toleranceOperator?.toString().trim() || "",
        toleranceTime: toleranceTime?.toString().trim() || "",
    ...
    serviceReport.TubeHousingLeakageOArm = testRecord._id;
    await serviceReport.save({ session });
