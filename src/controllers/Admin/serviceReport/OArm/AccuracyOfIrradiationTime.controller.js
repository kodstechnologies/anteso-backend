// controllers/Admin/serviceReport/OArm/AccuracyOfIrradiationTime.controller.js
import mongoose from "mongoose";
import AccuracyOfIrradiationTimeOArm from "../../../../models/testTables/OArm/AccuracyOfIrradiationTime.model.js";
...
const create = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    let testRecord = await AccuracyOfIrradiationTimeOArm.findOne({ serviceId }).session(session);
    ...
    if (testRecord) {
      ...
      testRecord = new AccuracyOfIrradiationTimeOArm({
        serviceId,
    ...
    serviceReport.AccuracyOfIrradiationTimeOArm = testRecord._id;
    await serviceReport.save({ session });
    ...
    console.error("AccuracyOfIrradiationTime O-Arm Create Error:", error);
...
const getById = asyncHandler(async (req, res) => {
  ...
  try {
    const testRecord = await AccuracyOfIrradiationTimeOArm.findById(testId).lean();
    if (!testRecord) {
...
const update = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const testRecord = await AccuracyOfIrradiationTimeOArm.findById(testId).session(session);
    if (!testRecord) {
      await session.abortTransaction();
      return res.status(404).json({ success: false, message: "Test record not found" });
    }
...
const getByServiceId = asyncHandler(async (req, res) => {
  ...
  try {
    const testRecord = await AccuracyOfIrradiationTimeOArm.findOne({ serviceId }).lean();
