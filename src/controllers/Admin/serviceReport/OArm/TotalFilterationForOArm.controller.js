// controllers/TotalFilterationForOArmController.js
import { asyncHandler } from "../../../../utils/AsyncHandler.js";
import TotalFilterationForOArm from "../../../../models/testTables/OArm/TotalFilteration.model.js";
...
const create = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    let testRecord = await TotalFilterationForOArm.findOne({ serviceId }).session(session);
    ...
    serviceReport.TotalFilterationForOArm = testRecord._id;
    await serviceReport.save({ session });
    ...
    console.error("TotalFilterationForOArm Create Error:", error);
...
const update = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const testRecord = await TotalFilterationForOArm.findById(testId).session(session);
    if (!testRecord) {
      await session.abortTransaction();
    ...
    if (session) await session.abortTransaction();
    console.error("TotalFilterationForOArm Update Error:", error);
    return res.status(500).json({
      success: false,
...
const getById = asyncHandler(async (req, res) => {
  ...
  try {
    const testRecord = await TotalFilterationForOArm.findById(testId).lean();
    if (!testRecord) {
      return res.status(404).json({ success: false, message: "Test record not found" });
    }
...
const getByServiceId = asyncHandler(async (req, res) => {
  ...
  try {
    const testRecord = await TotalFilterationForOArm.findOne({ serviceId }).lean();
